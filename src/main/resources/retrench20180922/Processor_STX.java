/*
Project 1 - Processing element for STX
...
public class Processor_STX {
  //CONSTANTS
  // debug constants
  private static final int MODULE_DEBUG_FLAG = 1;

  public static int executeSTX(int[] instruction_fields, int remaining_ticks, Processor processor) {
    int branch_id = -1;
    ...
    switch (branch_id) {
      ...
      case 1:
        ...
        // R --> LRR --> LRR + idx --> MAR |--> MBR
        if (MODULE_DEBUG_FLAG == 1) {
          System.out.println("Error!  Bad logic. Can't index on STX.");
        }
        if (remaining_ticks_return <= 0) {
          // first pass so specify number of clock cycles required to complete the operation
      ...
      default:
        ...
        processor.set_memory = 0;
        if (MODULE_DEBUG_FLAG == 1) {
          System.out.println("STX of type " + branch_id + " is not valid.");
        }
        break;
      // end branch switch
    }
